...
`.partial`, decoded independently, compared with source frames and audio samples, and
...
current output digest agree. `verify` performs a fresh full decode, not just a digest
check. A source, option, tool/detector version or imported-index change requires a new
